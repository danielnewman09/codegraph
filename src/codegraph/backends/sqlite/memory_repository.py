"""SqliteMemoryRepository — SQLite implementation of the MemoryRepository
abstract interface.

All SQL is sealed inside ``SqliteMemoryOps`` + delegated graph ops.
"""
...
class SqliteMemoryRepository(MemoryRepository):
    ...
    def __init__(
        ...
        self._memory_ops = SqliteMemoryOps(conn)
    ...
    def find_for_code_node(self, uid: str) -> list[dict]:
        return self._memory_ops.find_related_nodes(
            uid,
            SqliteMemoryOps.MEMORY_REL_PATTERN,
            source_labels=SqliteMemoryOps.MEMORY_LABELS,
        )
    ...
    def search_content(
        ...
        results = self._graph.search_fulltext(
            ...
            labels=SqliteMemoryOps.MEMORY_LABELS,
            tag=tag,
            limit=limit,
        )
        output: list[dict] = []
