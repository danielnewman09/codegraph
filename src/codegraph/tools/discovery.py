...
def handle_search_symbols(ctx: CodeGraphDispatcher, tool_input: dict) -> str:
    ...
    try:
        ...
        log.warning("search_symbols: Neo4j query failed", exc_info=True)
    ...
    try:
        with ctx.session() as session:
            ...
            result = session.run(
                ...
                "RETURN n.qualified_name AS qn, n.name AS name, "
                "n.kind AS kind, n.source AS source, "
                "n.brief_description AS brief", parameters={"qname": qname},
            )
            ...
            members_result = session.run(
                ...
                "ORDER BY m.kind, m.name", parameters={"qname": qname},
            )
            members: list[dict] = []
    ...
    try:
        with ctx.session() as session:
            result = session.run(
                ...
                "m.brief_description AS brief", parameters={"qname": qname},
            )
            record = result.single()
            if not record:
        ...
        log.warning("get_member: Neo4j query failed", exc_info=True)
        return json.dumps({"error": f"Query failed for {qname}"})
...
def handle_browse_namespace(ctx: CodeGraphDispatcher, tool_input: dict) -> str:
    ...
    try:
        ...
        log.warning("browse_namespace: Neo4j query failed", exc_info=True)
...
def handle_list_sources(ctx: CodeGraphDispatcher, _tool_input: dict) -> str:
    ...
    try:
        ...
        log.warning("list_sources: Neo4j query failed", exc_info=True)
    ...
    try:
        with ctx.session() as session:
            ...
            parents_result = session.run(
                ...
                "RETURN p.qualified_name AS qn, p.kind AS kind", parameters={"qname": qname},
            )
            parents = [
                {"qualified_name": r["qn"], "kind": r["kind"]}
                for r in parents_result
            ]
            ...
            children_result = session.run(
                ...
                "RETURN c.qualified_name AS qn, c.kind AS kind", parameters={"qname": qname},
            )
            children = [
                {"qualified_name": r["qn"], "kind": r["kind"]}
