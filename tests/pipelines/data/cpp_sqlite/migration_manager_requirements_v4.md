...
The `MigrationManager` class shall provide a schema versioning system for SQLite databases using the cpp-sqlite library. It shall track applied migrations, apply pending migrations in version order, detect schema drift, and roll back migrations when requested. The manager shall integrate with the existing cpp_sqlite::`Database` class without modifying it. Migrations shall be defined by the user as subclasses of the `Migration` abstract base class and registered with the manager. The manager shall provide a `verify` method that checks whether the current database schema matches the expected state from all applied migrations.

### Types defined by this requirement

**`Migration`** — Abstract base class for user-defined migrations:
- `int version;` — Migration version number, used for ordering and duplicate detection.
- `virtual void up(cpp_sqlite::Transaction&) = 0;` — Apply the migration.
- `virtual void down(cpp_sqlite::Transaction&) = 0;` — Roll back the migration.
- `virtual std::string get_description() const;` — Human-readable description.

**`MigrationErrorCode`** — Enum for error signaling:
- `DuplicateVersion` — A migration with this version is already registered.
- `MigrationFailed` — A migration's `up()` threw during apply.
- `RollbackFailed` — A migration's `down()` threw during rollback.
- `VersionNotFound` — The rollback target matches no applied migration and is greater than the lowest applied version.
- `NotInitialized` — No migration has ever been successfully applied by this manager instance (fresh database).
...
- `MissingTable` — An expected table is absent. (Reserved; not emitted by the current verify algorithm.)
- `ExtraTable` — An unexpected table is present. (Reserved; not emitted by the current verify algorithm.)
- `ColumnDifference` — A table's columns differ from expected. (Reserved; not emitted by the current verify algorithm.)
...
**`SchemaMismatch`** — Struct describing one schema drift:
- `int version;` — The migration version this mismatch relates to.
- `MismatchKind kind;` — What kind of drift was detected.
- `std::string detail;` — Human-readable description of the mismatch.

**`SchemaVersion`** — Value object for the `schema_versions` tracking table:
- `int version;` — Migration version number.
- `std::string applied_at;` — ISO-8601 timestamp of application.
- `std::string checksum;` — SHA-256 hash of the schema after applying this migration.

**`SchemaVerificationResult`** — Result type returned by `verify()`:
- `std::vector<SchemaMismatch> mismatches;` — Detected schema mismatches (empty if consistent).
- `bool is_consistent;` — True if no mismatches were found.
...
The `MigrationManager` shall provide an `apply` method that returns a `MigrationResult`. On first invocation, `apply` shall create the `schema_versions` table if it does not exist, in its own cpp_sqlite::`Transaction` that commits before any migration's `up()` runs (so a failed first migration leaves the empty table in place). It shall execute all unapplied migrations in version order against a cpp_sqlite::`Database` instance. Each migration's `up` method shall be invoked with a cpp_sqlite::`Transaction` reference. The manager shall record each successfully applied migration as a `SchemaVersion` in the `schema_versions` table (version: INTEGER, applied_at: TEXT ISO-8601, checksum: TEXT SHA-256). The `SchemaVersion` INSERT and the migration's `up()` shall execute within a single cpp_sqlite::`Transaction` — both succeed or both roll back atomically. If any migration's `up` method throws, the manager shall roll back the current migration's changes via the active `Transaction` (without affecting previously applied migrations) and return `MigrationResult{success=false, error=MigrationErrorCode::MigrationFailed}`. The manager shall not re-apply migrations already recorded in `schema_versions`. The manager shall compute each recorded `SchemaVersion.checksum` as the SHA-256 hash of the database schema — the concatenated SQL strings (joined with a single newline separator, in row order) from `SELECT sql FROM sqlite_master WHERE type IN ('table','index','trigger') ORDER BY name`, the same query, separator, and hashing that `verify` uses — captured immediately after the migration's `up()` succeeds, within the same `Transaction` as the `SchemaVersion` INSERT. If no migrations are pending (none registered, or every registered version is already recorded in `schema_versions`), `apply` shall create the `schema_versions` table if it does not exist and return `MigrationResult{success=true}` without executing any migrations. Concurrent apply calls are not supported; behavior is undefined if apply is called concurrently.
- tags: requirements
...
The `MigrationManager` shall provide a `verify` method that checks whether the current database schema matches the expected state from all applied migrations. The verify method shall compute a SHA-256 checksum of the current schema by querying `SELECT sql FROM sqlite_master WHERE type IN ('table','index','trigger') ORDER BY name` and hashing the concatenated SQL strings (joined with a single newline separator, in row order). It shall compare this checksum against the stored checksum of the **highest-applied** `SchemaVersion` in `schema_versions`. It shall return a `SchemaVerificationResult` containing a `std::vector<SchemaMismatch>` and an `is_consistent` boolean. If the live checksum matches the stored checksum, the result shall be `{mismatches={}, is_consistent=true}`. If they differ, the result shall contain one `SchemaMismatch` reporting the version number, `MismatchKind::ChecksumMismatch`, and a human-readable detail string. If `schema_versions` is empty, or if the `schema_versions` table does not exist (fresh database), `verify` shall return `{mismatches={}, is_consistent=true}` without creating the table. The current verify algorithm emits only `MismatchKind::ChecksumMismatch`; the other `MismatchKind` values (`MissingTable`, `ExtraTable`, `ColumnDifference`) are reserved for future finer-grained drift detection and are not produced by this algorithm. Verify is not required to be thread-safe; its behavior is undefined if called concurrently with `apply` or `rollback`.
- tags: requirements
...
The `MigrationManager` shall provide a `rollback` method that accepts a target `int` version and returns a `MigrationResult`. It shall roll back all migrations above the target version in descending version order. Each migration's `down` method shall be invoked with a cpp_sqlite::`Transaction` reference. Each rolled-back migration's `down()` invocation and its `schema_versions` row deletion shall execute within a single cpp_sqlite::`Transaction`, which is committed before the next migration's `down()` runs. If any `down` method throws, the manager shall abort the rollback: the throwing migration's `Transaction` is rolled back (its `schema_versions` row remains), previously rolled-back migrations remain rolled back, and `rollback` returns `MigrationResult{success=false, error=MigrationErrorCode::RollbackFailed}`. Target resolution shall follow this precedence: (1) If no migration has ever been successfully applied, `rollback` returns `MigrationResult{success=false, error=MigrationErrorCode::NotInitialized}`; the manager shall derive this state from the `schema_versions` table — at construction the manager is considered initialized if the table exists and contains at least one row, and after construction the state becomes initialized when `apply` first records a `SchemaVersion` row, never being reset by `rollback`. (2) If the manager is initialized but `schema_versions` is empty (possible after a full rollback), `rollback` returns `MigrationResult{success=true}` (no-op) for any target. (3) If the target version is less than or equal to the lowest applied version in `schema_versions`, `rollback` performs a full rollback: it shall roll back all applied migrations in descending version order and remove all rows from `schema_versions`, returning `MigrationResult{success=true}`. (4) If the target version matches an applied version in `schema_versions`, `rollback` shall roll back all applied versions strictly above the target in descending order, removing their rows, and return `MigrationResult{success=true}`. (5) Otherwise — the target is greater than the lowest applied version but matches no applied version — `rollback` returns `MigrationResult{success=false, error=MigrationErrorCode::VersionNotFound}`. Rollback is idempotent: calling `rollback(N)` a second time when no migrations are above N returns `MigrationResult{success=true}` (no-op). Rollback is not required to be thread-safe; its behavior is undefined if called concurrently with `apply`, `rollback`, or `verify`.
- tags: requirements
