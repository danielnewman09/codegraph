...
- `VersionNotFound` — The target version for rollback is not in schema_versions.
- `NotInitialized` — The manager has no migrations registered or applied (applies to idempotent verify/rollback on empty state).
...
- `ExtraTable` — An unexpected table is present.
- `ColumnDifference` — A table's columns differ from expected.
...
The `MigrationManager` shall provide a `rollback` method that accepts a target `int` version and returns a `MigrationResult`. It shall roll back all migrations above the target version in descending version order. Each migration's `down` method shall be invoked with a cpp_sqlite::`Transaction` reference. Rollback shall remove rows for rolled-back migrations from `schema_versions`. If any `down` method throws, the manager shall abort the rollback and leave the database in its partially-rolled-back state, returning `MigrationResult{success=false, error=MigrationErrorCode::RollbackFailed}`. If the target version does not exist in `schema_versions`, the manager shall return `MigrationResult{success=false, error=MigrationErrorCode::VersionNotFound}`. If `schema_versions` is empty, `rollback` shall return `MigrationResult{success=false, error=MigrationErrorCode::NotInitialized}`. Calling `rollback` to a version at or below the lowest applied version (equivalent to full rollback) is valid and shall remove all applied migrations, returning `MigrationResult{success=true}`. Rollback is idempotent — calling `rollback(N)` a second time when no migrations are above N shall return `MigrationResult{success=true}` (no-op).
- tags: requirements
...
- `Database Migration Manager` → `SchemaMismatch` **composes** (class)
- `Database Migration Manager` → `MismatchKind` **composes** (enum)
- `Database Migration Manager` → `MigrationErrorCode` **composes** (enum)
- `Database Migration Manager` → `MigrationResult` **composes** (class)
- `Database Migration Manager` → `SchemaVerificationResult` **composes** (class)

## Class: `SchemaMismatch`
- kind: class
- qualified_name: SchemaMismatch
- tags: scaffold
- description: Describes a single schema mismatch detected during verification

## Enum: `MismatchKind`
- kind: enum
- qualified_name: MismatchKind
- tags: scaffold
- description: Kinds of schema mismatches (MissingTable, ExtraTable, MissingColumn, ExtraColumn, ColumnTypeMismatch, ChecksumMismatch)

## Enum: `MigrationErrorCode`
- kind: enum
- qualified_name: MigrationErrorCode
- tags: scaffold
- description: Error codes for migration operations (Success, DuplicateVersion, MigrationFailed, RollbackFailed, VersionNotFound)

## Class: `MigrationResult`
- kind: class
- qualified_name: MigrationResult
- tags: scaffold
- description: Result type for migration operations with success flag, error code, and optional error message

## Class: `SchemaVerificationResult`
- kind: class
- qualified_name: SchemaVerificationResult
- tags: scaffold
- description: Result of schema verification containing a list of SchemaMismatch entries
