"""Codegen run against the migrated cpp-sqlite one-hop as-built graph.

The fixture in this repository's ``tests/unit_test_data`` directory is the
serialized as-built LayerGraph that the lifted cpp-sqlite integration producer
exports from its local fixture project (``tests/indexing/preservation/
cpp_sqlite_integration/`` → ``LayerGraph.from_backend(
...
(``cpp_sqlite_preservation_one_hop_impl.json`` — the same graph serialized with
...
Counts below are pinned to the Codegraph-owned producer fixture.
"""
...
_UNIT_TEST_DATA = _CODEGRAPH_ROOT / "tests" / "unit_test_data"

# These exports are generated by the lifted cpp-sqlite integration producer.
# The preservation prefix keeps this fixture family separate from older Codegraph
# inspection artifacts with the same historical names.
FIXTURE = _UNIT_TEST_DATA / "cpp_sqlite_preservation_one_hop.json"
...
#: ``body``/``body_file`` and the include spelling on INCLUDES edges.  Produced by
#: the lifted cpp-sqlite integration producer with the export flag set.
IMPL_FIXTURE = _UNIT_TEST_DATA / "cpp_sqlite_preservation_one_hop_impl.json"

# The lifted producer records paths relative to its Codegraph-owned fixture
# location.  Codegen's portable output contract retains the historic
# ``tests/fixtures`` layout, so normalize that relocation prefix at the
# consumer boundary.  Identity-sensitive tests use the raw export.
...
def _read_export(path: Path) -> list[dict]:
    return _portable_fixture_paths(json.loads(path.read_text(encoding="utf-8")))


def _read_raw_export(path: Path) -> list[dict]:
    return json.loads(path.read_text(encoding="utf-8"))

#: The original cpp-sqlite source files (committed Codegraph-owned copies)
#: that the byte-for-byte suite regenerates and compares against.
IMPL_SRC = (
...
class TestFixture:
    ...
    def test_deserializes_to_as_built_layer_graph(self):
        graph = LayerGraph.deserialize(_read_raw_export(FIXTURE))
    ...
    def test_endpoint_states_survive_all_field_modes(self):
        ...
        implementation_graph = LayerGraph.deserialize(_read_raw_export(IMPL_FIXTURE))
...
class TestImplementationExport:
    ...
    #: + the two gtest files).  Keep this declaration independent of the
    #: generated export so pytest can collect consumers before a separate
    #: producer session has populated inspection artifacts.
    ...
    def test_fixture_carries_implementation(self):
        ...
        data = _read_export(IMPL_FIXTURE)
...
def _load_impl() -> list[dict]:
    return _read_export(IMPL_FIXTURE)
...
class TestCli:
    def test_dry_run_plans_one_hop_graph(self, capsys):
        ...
        assert rc == 0
        assert (
            "tests/indexing/preservation/fixtures/cpp-sqlite/"
