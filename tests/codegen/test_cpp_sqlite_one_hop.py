...
#: residual-fragment work): 126 top-level entries, 472 flat nodes
...
class TestFixture:
    def test_is_nested_as_built_graph(self):
        ...
        assert sum("as-built" in e["tags"] for e in data) == 93
...
class TestGenerate:
    ...
    def test_external_deps_render_one_shell_each(self):
        ...
        for path in external:
            ...
            assert text.lstrip().startswith("#ifndef ")
            assert "// GENERATED by codegraph-codegen" not in text
            # Dependency FileNodes may retain empty namespace shells and
            # indexed includes, but no project declarations are composed.
            assert not any(
                line.lstrip().startswith(("class ", "struct ", "concept ", "using "))
...
class TestRenderedContent:
    ...
    def test_concepts_render_from_initializer(self):
        ...
        # As-built concepts carry template declarations separately from the
        # initializer; the renderer combines them into the declaration.
        assert "template<typename T>" in text
        assert (
            "concept TransferObject = "
    ...
    def test_source_files_preserve_includes_and_empty_namespace(self):
        text = generate(_load()).files[
            "tests/fixtures/cpp-sqlite/cpp_sqlite/src/cpp_sqlite/DBDataAccessObject.cpp"
        ]
        # As-built include discipline: the indexed spelling wins (WP3.3),
        # and the indexed otherwise-empty namespace shell is preserved.
